...
def cell_outlines(images_folder, files):
    ...
    print(f"Cellpose initialized with gpu={use_gpu}")
    print(f"Cellpose model running on device: {model.device}")
    ...
    for file in files:
        try:
            ...
            masks, flows, styles, diams = model.eval(
                imgs, diameter=25, channels=[0, 0], flow_threshold=0.4, do_3D=False
            )
            ...
            for center in centers:
                output.append(
                    {
                        "file": file,
                        "i": round(scale_x * center[0]),
                        "j": round(scale_y * center[1]),
                    }
                )
...
parser.add_argument(
    "dna_channel_substring",
    type=str,
    help="substring of filename to identify DNA channel",
)
